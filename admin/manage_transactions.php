...
// Total earnings query (sum of all completed transactions)
...
// Handle Search Query
$search_query = '';
$no_results = false; // Flag to check if there are no results
if (isset($_GET['search']) && !empty($_GET['search'])) {
    $search_term = $conn->real_escape_string($_GET['search']);
    $search_query = " AND u.name LIKE '%$search_term%'";

    // Transactions query with search filter
    $transactions = $conn->query("SELECT p.*, u.name AS customer_name, u.id AS customer_id, 
    ...
    if ($transactions->num_rows == 0) {
        $no_results = true; // Set the flag if no results are found
    }
} else {
    // Default transactions query when no search term is provided
    $transactions = $conn->query("SELECT p.*, u.name AS customer_name, u.id AS customer_id, 
                                  GROUP_CONCAT(DISTINCT li.name) AS item_names 
                                  FROM payments p 
                                  JOIN users u ON p.customer_id = u.id 
...
<div class="container">
    ...
    <?php else: ?>
        <table class="table">
            <tr><th>Order ID</th><th>Customer ID</th><th>Customer Name</th><th>Items</th><th>Amount</th><th>Payment Method</th><th>Status</th><th>Date</th></tr>
...
<style>
    /* Basic Styling for the Search Form and Buttons */
    .search-form {
        margin-bottom: 20px;
    }
